	...
	double		mjd)
{
	...
	pass = getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, sig.code, PHAS, bias[PHAS], bvar[PHAS]);
	...
	if(!pass)
{
		...
		{
			if(ft == F1 && sig.code != +E_ObsCode::L1C) getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L1C, bs, bsv, obs.satNav_ptr);
			if(ft == F1 && sig.code != +E_ObsCode::L2W) getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L2W, bs, bsv, obs.satNav_ptr);
		}
		
		if( obs.Sat.sys == +E_Sys::GAL)
		{
			if(ft == F1 && sig.code != +E_ObsCode::L1X) getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L1X, bs, bsv, obs.satNav_ptr);
			if(ft == F5 && sig.code != +E_ObsCode::L5X) getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L5X, bs, bsv, obs.satNav_ptr);
			if(ft == F5 && sig.code != +E_ObsCode::L5Q) getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L5Q, bs, bsv, obs.satNav_ptr);
			if(ft == F5 && bs[1]    ==        0.0     )	getBiasSinex(trace, obs.time, obs.Sat.id(), obs.Sat, E_ObsCode::L5I, bs, bsv, obs.satNav_ptr);
		}
		bias[1]=bs[1];
		bvar[1]=bsv[1];
	}
#endif
	tracepde(3, trace, " %.6f %s L%d biases for %3s      = %14.4f %14.4f\n",                       mjd, obs.Sat.id().c_str(), ft, sig.code._to_string(), bias[CODE], bias[PHAS]);
...
// 		}
		...
		if	(acsConfig.sat_pcv)
		{
			...
			if (pcvsat)
			{
				satantpcv(obs.rSat, rRec, *pcvsat, dAntSat, &satStat.nadir);
			}
			else
			{
		...
		for (auto& [ft, sig] : obs.Sigs)
		{
			...
			tracepde(lv, trace, "*---------------------------------------------------*\n");
			corr_meas(trace, obs, ft, satStat.el, dAntRec[ft], dAntSat[ft], satStat.phw, rec, mjd);
