	...
	for (int i = 0; i < E_Sys::_size(); i++)
	{
		...
		for (auto& Sat : sats)
		{
			...
			for (int tpredict = 0; tpredict <= acsConfig.ssrOpts.prediction_duration; tpredict += acsConfig.ssrOpts.prediction_interval)
			{
				...
				if	(  acsConfig.ssrOpts.clock_source == +E_Ephemeris::PRECISE
					|| acsConfig.ssrOpts.clock_source == +E_Ephemeris::SSR)
				{
					pass = satpos(trace, pTime, teph, obs, acsConfig.ssrOpts.clock_source,	E_OffsetType::APC, nav, pcoMap_ptr, false);
					if (pass == false)
					{
						BOOST_LOG_TRIVIAL(info) 
			...
			// Kalman filter clock
			if (acsConfig.ssrOpts.clock_source == +E_Ephemeris::KALMAN)
			{
				double clkVal = 0;
				bool pass = kfState.getKFValue({.type = KF::SAT_CLOCK, .Sat = Sat}, clkVal);
				if (pass == false)
				{
					BOOST_LOG_TRIVIAL(info) 
					<< Sat.id() << " failed ssrClk precise prediction.\n";
			...
			if (acsConfig.ssrOpts.code_bias_source == +E_Ephemeris::PRECISE)
			{
			...
			for (auto once : {1})
			if (acsConfig.ssrOpts.code_bias_source == +E_Ephemeris::SSR)
			{
			...
			for (auto once : {1})
			if (acsConfig.ssrOpts.phase_bias_source == +E_Ephemeris::SSR)
			{
