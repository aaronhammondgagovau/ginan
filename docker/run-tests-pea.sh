...
case $TEST_NUM in
  1)
    ...
    tracebase="$(basename $trace)";
    python ../scripts/diffutil.py   -i $trace -o "solutions/ex11/$tracebase"  --type trace --states_only --passthrough -a $ATOL;
    python ../scripts/diffutil.py   -i $trace -o "solutions/ex11/$tracebase"  --type trace --states_only;
    done
    ;;
  2)
