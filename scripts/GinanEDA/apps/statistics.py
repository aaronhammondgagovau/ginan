...
def dropdown_statistics(statistics_list):
    # print(statistics_list)
    return html.Div([
        dcc.Dropdown(
            id='statistics_dropdown_state',
            options=[{'label': i, 'value': i} for i in statistics_list],
...
def keys():
    a = db.MONGO_CL["Statisticss"].find_one()
    temp = list(a.keys())
...
@app.callback(
    [
        Output('statistics_dropdown_site',  'options'),
        Output('statistics_dropdown_sat',   'options'),
        Output('statistics_dropdown_key_x', 'options'),
        Output('statistics_dropdown_key_y', 'options'),
    ],
    inputs=[Input('state_dropdown_state',  'value')]
)
def update_dropdown(statistics):
    # print(statistics)
    if not statistics:
        return [],[],[],[]
    else:
        for st in db.DB_STATISTICS_DIC:
            if st['_id'] == statistics:
                # print (statistics)
                break
    # print(st)
    site_list = list(['ALL'])
...
@app.callback(
    ...
    statistics=[
        Statistics('statistics_dropdown_type',  'value'),
        Statistics('statistics_dropdown_state',  'value'),
        Statistics('statistics_dropdown_site',  'value'),
        Statistics('statistics_dropdown_sat',   'value'),
        Statistics('statistics_dropdown_key_x', 'value'),
        Statistics('statistics_dropdown_key_y', 'value'),
        Statistics('statistics_dropdown_site', 'options'),
        Statistics('statistics_dropdown_sat', 'options'),
        Statistics('slider-polynomial-degree', 'value'),
        Statistics('poly_dropdown', 'value'),
        Statistics('exclude_npt', 'value')
    ])
def update_graph_measurements(click, graph_type, Statistics, site, sat, xaxis, yaxis, list_site, list_sat, poly_deg, fit_type, exclude):
    # print("HELLO", graph_type, Statistics, "/",site,"/", sat,"/", xaxis,'/', yaxis)
    try:
        exclude = int(exclude)
    except:
        exclude = 0
    ...
    if graph_type is None or site is None or sat is None or xaxis is None or yaxis is None:
        ...
        site_, sat_, x_, y_ = db.get_series('Statistics', statistics, site, sat, xaxis, yaxis)
        trace = []
...
def layout():
    if db.MONGO_CL == None:
        ...
        return html.Div(
            [
                dropdown_type,
                dropdown_state(db.DB_STATISTICS),
